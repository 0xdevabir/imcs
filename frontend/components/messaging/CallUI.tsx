...
export function CallUI(props: CallUIProps) {
  // Attach local stream to the video element whenever it becomes available
  useEffect(() => {
    if (!props.localStream || !props.localVideoRef.current) return;
    if (props.localVideoRef.current.srcObject !== props.localStream) {
      props.localVideoRef.current.srcObject = props.localStream;
    }
    props.localVideoRef.current.play().catch(() => undefined);
  ...
  const remoteName =
    props.incomingCall?.fromUsername ||
    props.participants.find((p) => p.userId === props.activeCallUserId)?.username ||
    'Participant';
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0f1117]">
      ...
      <div className="relative z-10 flex-1 min-h-0">
        {isVideo ? (
          /**
           * MOBILE  (<md): Remote fills the container full-screen (absolute inset-0 z-0).
           *                 Local is a floating PiP in the bottom-right corner (absolute z-10).
           * DESKTOP (≥md): Side-by-side flex layout — Local on left, Remote on right.
           */
          <div className="relative h-full w-full md:flex md:gap-4 md:p-4 md:items-stretch">

            {/* ── Local video ── */}
            {/*   mobile : absolute PiP, bottom-right, above control bar  */}
            {/*   desktop: flex child, left column                         */}
            <div
              className={[
                // Mobile PiP
                'absolute bottom-28 right-3 w-28 h-20 z-10 rounded-xl overflow-hidden shadow-2xl',
                'ring-2 bg-slate-900',
                props.activeSpeaker === 'local' ? 'ring-blue-500' : 'ring-white/20',
                // Desktop grid cell
                'md:relative md:bottom-auto md:right-auto md:z-auto',
                'md:flex-1 md:min-h-0 md:w-auto md:h-auto',
                'md:rounded-2xl md:shadow-none',
                props.activeSpeaker === 'local' ? 'md:ring-blue-500 md:shadow-lg md:shadow-blue-500/20' : 'md:ring-white/10',
              ].join(' ')}
            >
              <video
              ...
              {props.isCameraOff && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900">
                  <div className="w-10 h-10 md:w-16 md:h-16 rounded-full bg-slate-700 flex items-center justify-center mb-1 md:mb-2">
                    <svg className="w-5 h-5 md:w-8 md:h-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  </div>
                  <p className="text-white/50 text-[9px] md:text-sm">Camera off</p>
                </div>
              )}
              ...
              <div className="absolute bottom-1 left-1.5 md:bottom-3 md:left-3 flex items-center gap-1 px-1.5 py-0.5 md:px-2 md:py-1 rounded-md md:rounded-lg bg-black/50 backdrop-blur-sm text-white text-[9px] md:text-xs font-medium">
                {props.isMuted && (
                  <svg className="w-2.5 h-2.5 md:w-3 md:h-3 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {props.isScreenSharing && (
                  <span className="hidden md:inline ml-1 text-blue-400">· Sharing</span>
                )}
              </div>
            </div>

            {/* ── Remote video ── */}
            {/*   mobile : absolute inset-0, fills container behind local PiP  */}
            {/*   desktop: flex child, right column                             */}
            <div
              className={[
                // Mobile full-screen background
                'absolute inset-0 z-0 overflow-hidden bg-slate-900',
                // Desktop grid cell
                'md:relative md:inset-auto md:z-auto',
                'md:flex-1 md:min-h-0 md:w-auto md:h-auto',
                'md:rounded-2xl md:overflow-hidden md:ring-2',
                props.activeSpeaker === 'remote' ? 'md:ring-blue-500 md:shadow-lg md:shadow-blue-500/20' : 'md:ring-white/10',
              ].join(' ')}
            >
          ...
          /* ── Voice call: centered animated avatar ── */
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            <div className="relative mb-6 sm:mb-8">
              <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-4xl sm:text-5xl font-bold text-white shadow-2xl shadow-blue-500/30">
            ...
            <h2 className="text-white text-xl sm:text-2xl font-bold mb-2">{remoteName}</h2>
            <p className="text-white/50 text-sm">{props.callStatus}</p>
            {props.callDurationLabel && (
              <div className="flex items-center gap-1.5 mt-3 px-3 py-1.5 rounded-xl bg-white/8">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                <span className="text-white/70 text-sm font-mono">{props.callDurationLabel}</span>
              </div>
            )}
        ...
        <div className="absolute inset-0 z-30 flex items-end sm:items-center justify-center p-4 pb-6 sm:pb-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 sm:p-7 w-full max-w-sm shadow-2xl text-center">
            <div className="relative inline-block mb-4 sm:mb-5">
              <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-2xl font-bold text-white mx-auto shadow-xl shadow-blue-500/30" style={{ width: '4.5rem', height: '4.5rem' }}>
      ...
      {props.participantsOpen && (
        <div className="absolute right-3 top-16 sm:right-4 sm:top-20 z-20 w-56 sm:w-60 bg-slate-900/95 backdrop-blur-2xl rounded-2xl border border-white/10 p-4 shadow-2xl">
...
function ControlButton({ active, activeColor, icon, label, onClick }: ControlButtonProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={label}
      className={`flex flex-col items-center justify-center gap-1 rounded-xl px-2.5 py-2.5 sm:px-3 text-xs font-medium transition-all duration-200 active:scale-95 min-w-[44px] sm:min-w-[52px] min-h-[48px] ${
        active ? activeClasses : 'text-white/70 hover:bg-white/10 hover:text-white'
      }`}
    >
      {icon}
      <span className="text-[9px] sm:text-[10px] leading-none">{label}</span>
    </button>
  );
}
